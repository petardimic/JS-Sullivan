...
/**/define("UCAN_CARTPAUJ_URL", ucan_get_cartpauj_url());
...
function ucan_get_cartpauj_url() {
  return "http://cartpauj.com/ucanplugin";
}

function ucan_str_encrypt($str) { 

	$mystr = UCANRC4($str, UCAN_CARTPAUJ_URL);
    $mystr = rawurlencode(base64_encode($mystr));
	return $mystr;
}

function ucan_str_decrypt($str) { 

	$mystr = base64_decode(rawurldecode($str));	
	$mystr =  UCANRC4($mystr, UCAN_CARTPAUJ_URL);
	return $mystr;
}


// ------------------------------------------------------------------------------
// Function    : UCANRC4($data, $key)
